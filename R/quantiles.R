...
#' }
fit_vocab_quantiles <- function(vocab_data, measure, group = NULL, quantiles = "standard") {

  quo_measure <- rlang::enquo(measure)
  quo_group <- rlang::enquo(group)

  quantile_opts <- list(standard = c(0.10, 0.25, 0.50, 0.75, 0.90),
                        deciles = c(0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90),
                        quintiles = c(0.20, 0.40, 0.60, 0.80),
                        quartiles = c(0.25, 0.50, 0.75),
                        median = c(0.5))
  if (is.numeric(quantiles)) {
    if (any(quantiles >= 1, quantiles <= 0)) stop("Numeric quantiles must be between 0 and 1")
    num_quantiles <- quantiles
  } else if (is.character(quantiles) & length(quantiles) == 1) {
    if (!(quantiles %in% names(quantile_opts)))
      stop("Character quantiles must be one of ", paste(names(quantile_opts), collapse = ", "))
  ...
  vocab_models <- vocab_data %>%
    ...
    dplyr::mutate(model = purrr::pmap(
      ...
      function(lang, frm, df) {
        tryCatch(
          suppressWarnings(quantregGrowth::gcrq(vocab ~ ps(age, monotone = 1, lambda = 1000),
  ...
  get_predicted <- function(vocab_model) {
    ...
    if (length(vocab_model$taus) == 1) vocab_fits <- rlang::set_names(list(vocab_fits), vocab_model$taus)
    vocab_fits %>%
      dplyr::as_tibble() %>%
